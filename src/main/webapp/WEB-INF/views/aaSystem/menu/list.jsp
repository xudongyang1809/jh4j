...
<body>
<c:if test="${menu.program ne null}">
	<div class="jumbotron">选中的节点是分配的程序末端节点，无法进行操作</div>	
</c:if>
<c:if test="${fatherId eq null}">
	<div class="jumbotron">选中的菜单节点不存在或被删除</div>
</c:if>
<c:if test="${fatherId ne null and menu.program eq null}">
	<div>
		<div class="tab-header">
			<form class="layui-form" action="${ctx}/menu/nodeList?id=${fatherId}" onsubmit="return navTabSearch(this, '#menu-list-view')">
		...
		<div class="tab-bars">
			<shiro:hasPermission name="Module:save">
				<a class="add" target="dialog" width="500" height="450" dialogId="createMenu" href="${ctx}/menu/preCreate?fatherId=${fatherId}">新增</a>
			</shiro:hasPermission>
