...
<form id="pagerForm" method="post" action="${ctx}/miAcceptanceM/list" onsubmit="return navTabAjaxDone(this)">
    ...
    <input type="hidden" name="strPoNo" value="${miAcceptanceM.strPoNo}"/>
    <input type="hidden" name="strStatus" value="${miAcceptanceM.strStatus}"/>
</form>
<div class="tab-header">
    <form method="post" action="${ctx}/miAcceptanceM/list" class="layui-form" onsubmit="return navTabSearch(this)">
        <div class="layui-form-item">
            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <select name="strInspType" class="layui-input">
                        <option value="" ${miAcceptanceM.strInspType eq '' ? 'selected':''}>-请选择-</option>
                        <option value="A" ${miAcceptanceM.strInspType eq 'A' ? 'selected':''}>采购入库</option>
                        <option value="B" ${miAcceptanceM.strInspType eq 'B' ? 'selected':''}>产品入库</option>
                        <option value="C" ${miAcceptanceM.strInspType eq 'C' ? 'selected':''}>贸易入库</option>
                    </select>
                </div>
            </div>
     <%--       <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <select  name="strLocNo" id="strLocNo" lay-verify="required">
            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="hidden" id="PickListNameID" name="strInUserId"/>
                    <input type="text" readonly id="PickListName" placeholder="点击挑选人员..." class="layui-input"
...
<div class="tab-bars">
    <shiro:hasPermission name="MiAcceptanceM:create">
        <a class="add" width="700" height="400" target="dialog" dialogId="createMiAcceptanceMDialog"
           href="${ctx}/miAcceptanceM/preCreate">添加</a>
    </shiro:hasPermission>
    <shiro:hasPermission name="MiAcceptanceM:edit">
        <a class="edit" target="dialog" max="true" mask="true" dialogId="upadteMiAcceptanceMDialog"
           href="${ctx}/miAcceptanceM/preUpdate/{slt_uid}">编辑</a>
    </shiro:hasPermission>
    <a class="delete" onclick="deleteList()">删除</a>
    <a class="confirm" target="ajaxTodo" href="${ctx}/miAcceptanceM/updatestatue/{slt_uid}?flg=C"><span>确认</span></a>
    <a class="cancelConfirm" target="ajaxTodo"
       href="${ctx}/miAcceptanceM/updatestatue/{slt_uid}?flg=N"><span>取消确认</span></a>
</div>
<div class="tab-content layui-form">
    <table class="frog-table">
        ...
        <tr>
            <td style="width: 20px;">
                <input type="checkbox" lay-skin="primary" class="checkAllInviter" lay-filter="checkboxInviter"></td>
            <th>序号</th>
            <th>入库单号</th>
            <th>入库类型</th>
            <th>订单/合同编号</th>
...
<%--                <td>${miAcceptanceM.strRedRush}</td>
...
<script type="text/javascript">
    ...
    function selectSingleUserCallback(userId, userNo, username) {
        $('#PickListNameID').val(userId);
        $('#PickListName').val(username);
    }
    function deleteList() {
        var ids = "";
        var flag = false;
        $('.frog-table', NavTab.getCurrentPanel()).find('tbody input[type="checkbox"]').each(function () {
            if ($(this).prop("checked")) {
                ...
                var stasus = $(this).parents('tr:first').attr("title");
                if (stasus != "N"){
                    flag = true;
                }
                if (ids == "") {
        ...
        layer.confirm('确定删除？', function (index) {
            ...
            $.post("${ctx}/miAcceptanceM/delete", {id: ids}, function (msg) {
                layer.close(index);
                try {
